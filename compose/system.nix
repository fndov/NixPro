{ config, lib, inputs, pkgs, settings, ... }: {
  ...
  boot.loader.grub.enable = settings.system.bootMode != "uefi";
  boot.loader.grub.useOSProber = true;
  boot.loader.systemd-boot.enable = settings.system.bootMode == "uefi";
  boot.loader.systemd-boot.editor = false;
  boot.loader.efi.canTouchEfiVariables = settings.system.bootMode == "uefi";
...
} else {}) // (if settings.driver.graphics == "amd" then {
} else {}) // (if settings.driver.graphics == "intel" then {
  hardware.graphics.enable = true;
  hardware.graphics.enable32Bit = true;
} else {}) // (if settings.system.automation && settings.profile != "microsoft" then {
...
} else {}) // (if settings.system.security then {
...
} else {}) // (if settings.system.sshd then {
  services.openssh.enable = true;
  services.openssh.ports = [ 22 ];
  services.openssh.settings.UseDns = true;
  services.openssh.settings.PasswordAuthentication = true;
} else {}) // (if settings.system.printing then {
  services.printing.enable = true;
  services.avahi.enable = true;
  services.avahi.nssmdns4 = true;
  services.avahi.openFirewall = true;
  environment.systemPackages = [ pkgs.cups-filters ];
} else {}) // (if settings.driver.bluetooth then {
  hardware.bluetooth.enable = true;
  services.blueman.enable = true;
} else {}) // (if settings.driver.networking && settings.profile != "microsoft" then {
